...
app.post("/mcp", (req, res) => {
  ...
  if (method === "initialize") return res.json({jsonrpc:"2.0",id,result:{protocolVersion:"2025-03-26",capabilities:{tools:{}},serverInfo:{name:"svakom",version:"1.0.0"}}});
  if (method === "notifications/initialized") return res.json({jsonrpc:"2.0",id:null,result:{}});
  if (method === "tools/list") return res.json({jsonrpc:"2.0",id,result:{tools:[
    ...
    {name:"toy_set_speed",description:"设置强度0到1",inputSchema:{type:"object",properties:{speed:{type:"number"}},required:["speed"]}},
    {name:"toy_set_pattern",description:"振动花样1到8",inputSchema:{type:"object",properties:{pattern:{type:"integer"},level:{type:"number"}},required:["pattern","level"]}},
